...
describe("CategoryTabs", () => {
  ...
  describe("Rendering", () => {
    ...
    it("should render all category tabs", () => {
      ...
      expect(
        screen.getByRole("button", { name: /JSX 基础/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /组件开发/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /React Hooks/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("Active State", () => {
    ...
    it("should update active tab when activeCategory changes", () => {
      ...
      expect(screen.getByRole("button", { name: /JSX 基础/i })).toHaveClass(
        "active",
      );

      rerender(<CategoryTabs {...defaultProps} activeCategory="hooks" />);

      expect(screen.getByRole("button", { name: /JSX 基础/i })).not.toHaveClass(
        "active",
      );
      expect(screen.getByRole("button", { name: /React Hooks/i })).toHaveClass(
        "active",
      );
    });
  });
  ...
  describe("User Interactions", () => {
    it("should call onCategoryChange when tab is clicked", async () => {
      ...
      render(
        <CategoryTabs {...defaultProps} onCategoryChange={handleChange} />,
      );
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle single category", () => {
      ...
      expect(
        screen.getByRole("button", { name: /唯一分类/i }),
      ).toBeInTheDocument();
      expect(screen.getByRole("button", { name: /唯一分类/i })).toHaveClass(
        "active",
      );
    });
