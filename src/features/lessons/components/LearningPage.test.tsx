...
jest.mock('../../progress', () => ({
  TestResults: ({ results, isCorrect, isLoading }: { results: any[]; isCorrect: boolean; isLoading: boolean }) => (
    <div data-testid="test-results">
      {isLoading && <span>Loading...</span>}
      {!isLoading && <span>{isCorrect ? 'All tests passed' : 'Some tests failed'}</span>}
...
describe('LearningPage', () => {
  ...
  describe('Loading State', () => {
    it('should show loading spinner when loading', () => {
      ...
      expect(screen.getByText('', { selector: '.ant-spin' })).toBeInTheDocument()
    })
  ...
  describe('Rendering', () => {
    ...
    it('should render control buttons', () => {
      ...
      expect(screen.getByRole('button', { name: '重置代码' })).toBeInTheDocument()
      expect(screen.getByRole('button', { name: '查看答案' })).toBeInTheDocument()
      expect(screen.getByRole('button', { name: '保存代码' })).toBeInTheDocument()
    })
  })
