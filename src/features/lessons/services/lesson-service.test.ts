...
jest.mock('../repository/lesson-repository')
...
describe('LessonService', () => {
  ...
  describe('loadCategory', () => {
    it('should successfully load a category', async () => {
      ...
      expect(mockLessonRepository.loadCategory).toHaveBeenCalledWith('jsx-basics')
    })
  ...
  describe('getLesson', () => {
    it('should successfully load a lesson', async () => {
      ...
      expect(mockLessonRepository.loadLesson).toHaveBeenCalledWith('jsx-basics', 'lesson-1')
    })

    it('should return error when lesson not found', async () => {
      mockLessonRepository.loadLesson.mockResolvedValue(null)

      const result = await lessonService.getLesson('jsx-basics', 'non-existent')
  ...
  describe('preloadNextCategory', () => {
    it('should call repository preload method', async () => {
      ...
      expect(mockLessonRepository.preloadNextCategory).toHaveBeenCalledWith('jsx-basics')
    })

    it('should silently handle preload errors', async () => {
      const consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation()
      mockLessonRepository.preloadNextCategory.mockRejectedValue(new Error('Preload failed'))
  ...
  describe('preloadAdjacentCategories', () => {
    it('should call repository preload method', async () => {
      mockLessonRepository.preloadAdjacentCategories.mockResolvedValue(undefined)

      await lessonService.preloadAdjacentCategories('jsx-basics')

      expect(mockLessonRepository.preloadAdjacentCategories).toHaveBeenCalledWith('jsx-basics')
    })
  ...
  describe('clearCache', () => {
    ...
    it('should call repository clearCache with categoryId', async () => {
      ...
      expect(mockLessonRepository.clearCache).toHaveBeenCalledWith('jsx-basics')
    })
  })
})
