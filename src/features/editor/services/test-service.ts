...
class TestServiceImpl implements TestServiceAPI {
  ...
  private async testWithAST(
    ...
    const result = await codeAnalyzerService.evaluateChecker(
      code,
      testCase.checker,
    );
  ...
  private testWithPattern(code: string, testCase: TestCase): TestResult {
    ...
    try {
      ...
      return {
        ...
        message: includes
          ? "Pattern found"
          : `Pattern not found: ${testCase.pattern}`,
      };
    }
  }
  ...
  async validateCode(
    code: string,
  ): Promise<Result<CodeAnalysisResult, ServiceError>> {
    try {
      const errors: string[] = [];
      ...
      if (code.includes("var ")) {
        warnings.push(
          'Using "var" is discouraged, consider using "let" or "const"',
        );
      }
  ...
  async analyzeCodeQuality(
    code: string,
  ): Promise<Result<Record<string, unknown>, ServiceError>> {
    try {
   ...
   * 分析代码缩进
   */
  private analyzeIndentation(code: string): {
    consistent: boolean;
    style: string;
  } {
  ...
  private analyzeNaming(code: string): {
    ...
    // 检查变量名是否使用驼峰命名
    const varMatches =
      code.match(/\b(var|let|const)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g) || [];
    ...
    for (const match of varMatches) {
      ...
      // 检查是否为驼峰命名（首字母小写）
      const isCamelCase =
        /^[a-z][a-zA-Z0-9]*$/.test(name) || /^_[a-zA-Z0-9]*$/.test(name);
      if (isCamelCase) {
        compliantCount++;
      } else {
  ...
  private estimateComplexity(code: string): number {
    ...
    complexity +=
      ifCount + elseCount + caseCount + forCount + ternaryCount * 0.5;
