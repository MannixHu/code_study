  ...
  constructor() {
    ...
    this.version(2)
      ...
      .upgrade(async (tx) => {
        ...
        await tx.table('progress').toCollection().modify((progress: any) => {
 ...
 */
export async function getProgress(lessonId: string): Promise<ProgressV2 | undefined> {
  try {
 ...
 * @returns 该分类下的所有进度
 */
export async function getCategoryProgress(categoryId: string): Promise<ProgressV2[]> {
  try {
 ...
 */
export async function saveProgressBatch(progressList: ProgressV2[]): Promise<void> {
  try {
    // 确保所有项都有 categoryId
    const items = progressList.map((progress) => ({
      ...progress,
      categoryId: progress.categoryId || progress.lessonId.split('-').slice(0, -1).join('-'),
    }))
