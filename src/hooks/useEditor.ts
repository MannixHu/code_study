 ...
 * 使用编辑器的 Hook
 */
export function useEditor(lessonId: string, starterCode?: string): UseEditorResult {
  const { userCode, isSaved, setUserCode, resetCode, loadSavedCode, markAsSaved } =
    useEditorStore()
  ...
  useEffect(() => {
    if (lessonId) {
      loadSavedCode(lessonId).catch((error) => {
